...
class BasicBlock(nn.Module):
    def __init__(self, sizes):
        super().__init__()
        self.conv1 = nn.Conv2d(sizes[0], sizes[1], kernel_size=3, padding=1, bias=False) 
        self.bn1 = nn.BatchNorm2d(sizes[1])
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(sizes[2], sizes[3], kernel_size=3, padding=1, bias=False) 
        self.bn2 = nn.BatchNorm2d(sizes[3])
...
class UVNetCurveEncoder(nn.Module):
    def __init__(self, in_channels=6, output_dims=64):
        ...
        self.conv =nn.Sequential(
                    _conv1d(in_channels, 64, kernel_size=3, padding=1, bias=False),
                    _conv1d(64, 128, kernel_size=3, padding=1, bias=False),
                    _conv1d(128, 256, kernel_size=3, padding=1, bias=False)
        )
...
class UVNetSurfaceEncoder(nn.Module):
    def __init__(
        ...
        # self.bl1 = BasicBlock([128,128,128,128])
        # self.conv1 = _conv2d(in_channels, 64, 3, padding=1, bias=False)
        mid_dim = 128
        fin_dim = 256
        in_channels = 7
        self.conv =nn.Sequential(
                    _conv2d(in_channels, 64, 3, padding=1, bias=False),
                    _conv2d(64, 128, 3, padding=1, bias=False),
                    _conv2d(128, 256, 3, padding=1, bias=False)
        )
        
        
#         self.conv1 = nn.Sequential(
#                 nn.Conv2d(in_channels, mid_dim, kernel_size=3, padding=1, bias=False),
...
#         self.conv3 = _conv2d(mid_dim, mid_dim, 3, padding=1, bias=False)
#         self.conv4 = _conv2d(mid_dim, fin_dim, 3, padding=1, bias=False)
        # self.conv3 = _conv2d(128, 256, 3, padding=1, bias=False)
        self.final_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = _fc(fin_dim, output_dims, bias=False)
        for m in self.modules():
            self.weights_init(m)
