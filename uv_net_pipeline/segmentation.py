...
from uv_net_pipeline.uvnet.models import Segmentation, SegmentationTIZ
...
parser.add_argument(
    "--dataset",
    choices=("mfcad", "fusiongallery", "mfcadp"),
...
-----------------------------------------------------------------------------------
    ...
    val_data = Dataset(root_dir=args.dataset_path, split="val", random_rotate=args.random_rotate)
    # val_data = MFTestDataset(root_dir='/home/egor/data/janush_dataset/converted_20/', split="test",  random_rotate=args.random_rotate)

    train_loader = train_data.get_dataloader(batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    val_loader = val_data.get_dataloader(batch_size=1, shuffle=False, num_workers=args.num_workers)

    model = SegmentationTIZ(Dataset.num_classes(), val_data, crv_in_channels=args.crv_in_channels)
