...
from uv_net_pipeline.uvnet.models import Segmentation
...
-----------------------------------------------------------------------------------
    ...
    model = Segmentation(num_classes=Dataset.num_classes(), crv_in_channels=args.crv_in_channels)
    train_data = Dataset(root_dir=args.dataset_path, split="train", random_rotate=args.random_rotate)
    val_data = MFTestDataset(
        root_dir=args.test_dataset_path,
        split="test",
    ...
    print(f"Our_metric: {results[0]['test_tiz'] * 100.0}")
