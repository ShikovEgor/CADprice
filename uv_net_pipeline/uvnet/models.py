...
class MyMetr(Metric):
    ...
    def update(self, sample, labels):
        ...
        self.correct += torch.sum(get_mf_jaccard(sample=sample, labels=labels[sample["filename"]]["labels"]))
...
class UVNetClassifier(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.curv_encoder = uv_net_pipeline.uvnet.encoders.UVNetCurveEncoder(in_channels=6, output_dims=crv_emb_dim)
        self.surf_encoder = uv_net_pipeline.uvnet.encoders.UVNetSurfaceEncoder(in_channels=7, output_dims=srf_emb_dim)
...
class SegmentationTIZ(pl.LightningModule):
    """
    PyTorch Lightning module to train/test the segmenter (per-face classifier).
    """

    def __init__(self, num_classes, test_data, crv_in_channels=6):
        """
        Args:
            num_classes (int): Number of per-face classes in the dataset
        """
        ...
        self.valid_preds = dict()
        # self.test_tiz = torchmetrics.IoU(compute_on_step=False)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        preds = F.softmax(logits, dim=-1)
        # self.valid_preds[data["filename"]] = preds.cpu().numpy()
        return 0

    def validation_epoch_end(self, outs):
        jaccards = []
        for sample in self.test_dataset:
            if sample["filename"] in self.valid_preds:
                jaccards.append(
                    get_mf_jaccard(
                        sample=sample,
                        labels=self.valid_preds[sample["filename"]]["labels"],
                    )
                )

        self.log("test_tiz", np.mean(jaccards))
